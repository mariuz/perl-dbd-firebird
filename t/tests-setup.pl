...
Read the configurations from the L<test.conf.tmp> file.

If only the record for the isql path is found in the configuration, as
generated by Makefile.PL, than run I<after_automated>.

Else run I<after_interactive>.
...
=head2 after_automated

After automated setup with Makefile.PL, we have only the record for
the isql path in the configuration file.

Read the environment variables to get the rest of the required
parameters.
...
sub after_automated {
    ...
    unless (scalar keys %{$para} > 1) {
        print "Failed to get required parameters!\n";
        die "Set the environment variables, or run the interactive setup!\n"
    }
...
Save parameters to 'test.conf.tmp' for later use.
